<template>
  <header>
    <div v-if="siteWidth > 600" class="header-line header-lineUp">
      <address class="page-wrapper">
        <a
          v-for="(contact, key) in global.contacts"
          :href="contact.href"
          :target="contact.target"
          :rel="contact.target ? 'noreferred' : null"
          class="link link-redToWhite link-underline"
          :class="key === 'email' ? 'link-toLeft' : false"
          :key="key"
        >
          {{ contact.label }}
        </a>
    ...
    <div class="header-line header-lineDown">
      <div class="page-wrapper">
        <nav>
          <router-link to="/" class="link link-whiteToRed link-logo">
            <AppFlipper :label="isTitle" :size="48" :imageFront="'logo-front.svg'" :imageBack="'logo-back.svg'" />
          </router-link>
          <div v-if="siteWidth > 600" class="nav-categories">
            <router-link
              v-for="category in categories"
              :key="category.slug"
              :to="{ name: 'Category', params: { category: category.slug } }"
              class="link link-nav link-whiteToRed link-underline"
            >
        ...
        <menu>
          <InputSwitch
            v-if="siteWidth > 800"
            class="button button-switch"
            :options="currencies"
            @change="changeCurrency"
          />
          <button class="button button-nav" @click="showSearch">
            <svg x="0px" y="0px" viewBox="0 0 64 64" stroke="#fff" stroke-width="2" fill="transparent">
              <path d="M47.16 28.58A18.58 18.58 0 1 1 28.58 10a18.58 18.58 0 0 1 18.58 18.58zM54 54L41.94 42" />
            </svg>
          </button>
          <button v-if="siteWidth > 800" class="button button-nav" @click="showModal">
            <svg x="0px" y="0px" viewBox="0 0 64 64" stroke="#fff" stroke-width="2" fill="transparent">
              <path
          ...
          <button v-if="siteWidth <= 800" class="button button-nav excluded" @click="showAside('TheMobileMenu')">
            <svg x="0px" y="0px" viewBox="0 0 511 511" fill="#fff" class="icon-menu">
              <path
          ...
          <button class="button button-nav excluded" @click="showAside('CartWidget')">
            <svg x="0px" y="0px" viewBox="0 0 64 64" stroke="#fff" stroke-width="2" fill="transparent">
              <path d="M14 17.44h46.79l-7.94 25.61H20.96l-9.65-35.1H3"></path>
              <circle cx="27" cy="53" r="3"></circle>
              <circle cx="47" cy="53" r="3"></circle>
              <rect v-if="shoppingCart.length" x="26" y="8" width="22" height="22"></rect>
            </svg>
          </button>
        </menu>
      </div>
      <TheSearch v-show="isSearchOpened" @close="closeSearch" :isVisible="isSearchOpened" />
    </div>
    <template v-for="category in categories">
      <div v-if="category.offer" :key="category.id" class="header-line header-lineOffer">
        <router-link :to="{ name: 'Category', params: { category: category.slug } }" class="link link-offer">
          {{ category.offer }}
        </router-link>
      </div>
    </template>
    ...
    <TheAside
      v-show="isAsideOpened"
      @close="closeAside"
      @login="login"
      :isVisible="isAsideOpened"
      :component="component"
    />

    <AppModal v-show="isModalVisible" @close="closeModal" :isVisible="isModalVisible">
      <template v-slot:header>Come in, Buddy.</template>
      <template v-slot:body>
        <p>Sorry, Buddy, but signing up temporarily is not available.</p>
        <InputUser />
        <InputEmail />
      </template>
      <template v-slot:footer>
        <button type="button" class="button button-toCart button-disabled" disabled @click="closeModal">Log In</button>
      </template>
    </AppModal>
  </header>
</template>
...
// Component: QuickSearch Widget
import TheSearch from "@/components/core/TheSearch";
// Component: Aside
import TheAside from "@/components/core/TheAside";
// Component: Flipping Logo in Footer & Header
...
// Component: Currency Switcher in Header
...
export default {
  ...
  mixins: [resizeListener, AppModalMixin],
  data() {
    return {
  ...
  created() {
    this.currencies = this.global.currencies;
    if (localStorage.currency) {
      this.currencies.defaultTypeId = Number.parseFloat(localStorage.currency);
    } else {
      this.currencies.defaultTypeId = this.global.currencies.defaultTypeId;
    }
  },
  computed: {
    ...
    // Returns Site Title in Logo or Empty in case of Client Width
    isTitle() {
      return (this.siteWidth > 600 && this.siteWidth <= 1024) || this.siteWidth <= 440 ? "" : this.global.logo;
    },
  },
  methods: {
    ...mapActions("global", ["setCurrentCurrency"]),

    changeCurrency(payload) {
      const currencyId = payload === true ? 1 : 0;
      localStorage.currency = this.currencies.defaultTypeId = currencyId;
